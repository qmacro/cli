	...
	Context("when the environment is set up correctly", func() {
		...
		Context("when the app does exist", func() {
			Context("when the app is a buildpack app", func() {
				Context("when the app has only ONE buildpack", func() {
					var (
						domainName string
...
`, appName, appName, domainName, tcpDomain.Name))
					...
					Context("when the app is started and has 2 instances", func() {
						It("displays the app information with instances table", func() {
							...
							Eventually(session).Should(Say("buildpacks:\\s+staticfile_buildpack"))
...
`, appName))
							...
							Eventually(helpers.CF("push", appName, "-p", appDir, "-f", manifestPath)).Should(Exit(0))
						})
					})

					It("displays the app information with all buildpacks", func() {
						session := helpers.CF("app", appName)
						Eventually(session).Should(Say("buildpacks:\\s+staticfile_buildpack, ruby_buildpack"))
						Eventually(session).Should(Exit(0))
					})
				})
			})
